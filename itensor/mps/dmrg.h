...
#include "itensor/mps/localmposet_mps.h"
...
Real
dmrg(MPSt<Tensor>& psi,
	 const std::vector<MPOt<Tensor>>& H,
	 const std::vector<MPSt<Tensor> >& psis,
	 const Sweeps& sweeps,
	 const Args& args = Global::args())
	{
	LocalMPOSet_MPS<Tensor> PH(H,psis,args);
	Real energy = DMRGWorker(psi,PH,sweeps,args);
	return energy;
	}
...
//
//Three-site DMRG - the chain should be at least three site long !!
//
     ...
     const Args& args = Global::args())
{
    LocalMPO<Tensor> PH(H,args);
    PH.numCenter(3);
    Real energy = DMRGWorker3(psi,PH,sweeps,args);
    return energy;
}
     ...
     const Args& args = Global::args())
{
    LocalMPO<Tensor> PH(H,args);
    PH.numCenter(3);
    Real energy = DMRGWorker3(psi,PH,sweeps,obs,args);
    return energy;
}
...
//	std::ofstream densityData;
//	densityData.open("densityData");
//	densityData<<std::fixed;
//	densityData<<std::setprecision(10);
